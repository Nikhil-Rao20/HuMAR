...
# ***************************** Tokenize function **************************
class RobertaTokenizer(nn.Module):
    def __init__(self, ):
...
def single_tokenize(texts: Union[str, List[str]],
             context_length: int = 77,
             truncate: bool = False) -> torch.LongTensor:
    """
    Returns the tokenized representation of given input string(s)
    ...
    if isinstance(texts, str):
        texts = [texts]
    assert len(texts) == 1, "Only accept one text."
    sot_token = _tokenizer.encoder["<|startoftext|>"]
    eot_token = _tokenizer.encoder["<|endoftext|>"]
    all_tokens = [[sot_token] + _tokenizer.encode(text) + [eot_token]
                  for text in texts]
    ...
    if truncate:
        ...
        raise RuntimeError(
            f"Input {texts[0]} is too long for context length {context_length}"
        )
...
if __name__ == "__main__":
    text = ['a big dog']
    model = RobertaTokenizer()

    res = model(text)
    msk = res.attention_mask.ne(1).bool()
    print(res, msk, res['input_ids'].shape, msk.shape)